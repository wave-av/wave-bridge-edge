...
authorization, scope, entitlement, metering, and payment settlement are DELEGATED to the wave-gateway** <!-- # guard:allow architecture-doc -->
...
```
  client ──Authorization: Bearer <wave_key>──▶  wave-spoke (this worker)
                                                   ...
                                                   ▼
                                               wave-gateway  ── authorize · scope · entitlement · meter  # guard:allow architecture-doc
                                                   ▼
...
| 8 | AuthN / scope / entitlement / metering | ☐ delegated | Enforced by the **wave-gateway** (`authorize → scope → entitlement → rateLimit → meter`). | <!-- # guard:allow architecture-doc -->
...
until the wave-gateway is deployed in front of it. The gateway is a hard prerequisite: spoke <!-- # guard:allow architecture-doc -->
